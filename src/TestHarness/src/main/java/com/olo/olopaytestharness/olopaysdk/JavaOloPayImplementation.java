...
public class JavaOloPayImplementation implements ISDKImplementation {
    ...
    public static void initializeSdk(InitCompleteCallback callback) {
        ...
        initializer.setup(application, oloPayEnv, callback);
    }
    ...
    @Override
    @OptIn(markerClass = ExperimentalSerializationApi.class)
    public void submitGooglePay(@NonNull IGooglePayLauncher launcher, @NonNull IOloApiSettings apiSettings, @NonNull IUserSettings userSettings, @NonNull IGooglePaySettings googlePaySettings, int amount, List<GooglePayLineItem> lineItems, Basket basket) {
        _workerStatus.isBusy().setValue(true);

        _googlePayBasket = basket;
        _logger.logText(KotlinOloPayImplementation.GooglePaySubmitHeader);

        GooglePayCheckoutStatus checkoutStatus = (googlePaySettings.getUsePayNowGooglePayButton().getValue()) ? GooglePayCheckoutStatus.FinalImmediatePurchase : GooglePayCheckoutStatus.EstimatedDefault;
        String totalPriceLabel = googlePaySettings.getUseLineItems().getValue() ? "Total" : null;

        new MainThreadExecutor().execute(() -> {
            _logger.logBasket(basket);
            try {
                launcher.present(amount, checkoutStatus, totalPriceLabel, lineItems);
            } catch (Exception e) {
                _logger.logException(e);
                _workerStatus.isBusy().setValue(false);
            }
    ...
    @Override
    @OptIn(markerClass = ExperimentalSerializationApi.class)
    public void onGooglePayResult(@NonNull GooglePayResult result, @NonNull IOloApiSettings apiSettings, @NonNull IUserSettings userSettings) {
        _logger.logGooglePayResult(result);

        if (!apiSettings.getCompletePayment().getValue() || !(result instanceof GooglePayResult.Completed)) {
            _workerStatus.isBusy().setValue(false);
            return;
        }
        ...
        IPaymentMethod paymentMethod = ((GooglePayResult.Completed)result).getPaymentMethod();
        PaymentType paymentType = new PaymentType(paymentMethod);
