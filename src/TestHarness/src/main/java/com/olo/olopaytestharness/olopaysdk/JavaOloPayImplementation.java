...
public class JavaOloPayImplementation implements ISDKImplementation {
    ...
    @Override
    public void submitCvv(@NonNull Context context, @Nullable ICvvTokenParams params) {
    ...
    public void submitPayment(@NonNull Context context, @Nullable IPaymentMethodParams params, @NonNull IOloApiSettings oloApiSettings) {
                ...
                new MainThreadExecutor().execute(() -> {
                    ...
                    OloApiClientExtensionsKt.createBasketWithProductFromSettings(client, oloApiSettings, new ApiResultCallback<Basket>() {
                        ...
                        public void onSuccess(Basket basket) {
                            ...
                            submitBasket(client, basket, paymentMethod, oloApiSettings);
                        }
    ...
    @Override
    @OptIn(markerClass = ExperimentalSerializationApi.class)
    public void submitGooglePay(@NonNull IGooglePayContext context, @NonNull IOloApiSettings settings) {
        _workerStatus.isBusy().setValue(true);

        _googlePayBasket = null;
        _logger.logText(KotlinOloPayImplementation.GooglePaySubmitHeader);

        if (!settings.getCompletePayment().getValue()) {
            context.present();
            _workerStatus.isBusy().setValue(false);
            return;
        }

        OloApiClient client = OloApiClientExtensionsKt.createApiClientFromSettings(settings);
        OloApiClientExtensionsKt.createBasketWithProductFromSettings(client, settings, new ApiResultCallback<Basket>() {
            @Override
            public void onSuccess(Basket basket) {
    ...
    @Override
    @OptIn(markerClass = ExperimentalSerializationApi.class)
    public void onGooglePayResult(@NonNull Result result, @NonNull IOloApiSettings settings) {
        _logger.logGooglePayResult(result);

        if (!settings.getCompletePayment().getValue() || !(result instanceof Result.Completed)) {
            _workerStatus.isBusy().setValue(false);
            return;
        }
        ...
        OloApiClient client = OloApiClientExtensionsKt.createApiClientFromSettings(settings);
        submitBasket(client, _googlePayBasket, paymentMethod, settings);
    }

    @OptIn(markerClass = ExperimentalSerializationApi.class)
    private void submitBasket(OloApiClient client, Basket basket, IPaymentMethod paymentMethod, IOloApiSettings settings) {
        OloApiClientExtensionsKt.submitBasketFromSettings(client, settings, paymentMethod, basket.getId(), new ApiResultCallback<Order>() {
            @Override
