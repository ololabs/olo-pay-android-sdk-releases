...
class Logger : ILogger {
    ...
    override fun logGooglePayResult(result: GooglePayResult) {
        when (result) {
    ...
    override fun logUserSettings(settings: IUserSettings, context: Context, showBillingAccount: Boolean) {
        val appContext = context.applicationContext
        val builder = StringBuilder()

        builder.appendLine(UserSettingsHeader)
        builder.appendLine("${appContext.getString(R.string.user_settings_use_logged_in_user)}: ${settings.useLoggedInUser.value}")
        builder.appendLine("${appContext.getString(R.string.settings_user_email)}: ${settings.userEmail.value}")
        if(showBillingAccount) {
            builder.appendLine("${appContext.getString(R.string.settings_user_saved_card_billing_account)}: ${settings.savedCardBillingId.value}")
        }

        logText(builder.toString())
    }

    override fun logGooglePaySettings(settings: IGooglePaySettings, context: Context) {
        val appContext = context.applicationContext
        val builder = StringBuilder()

        builder.appendLine(GooglePaySettingsHeader)
        builder.appendLine("${appContext.getString(R.string.settings_google_pay_checkout_status_label)}: ${settings.usePayNowGooglePayButton.value}")
        builder.appendLine("${appContext.getString(R.string.settings_google_pay_show_line_items_label)}: ${settings.useLineItems.value}")
    ...
    companion object {
        ...
        private const val UserSettingsHeader: String = "----------- USER SETTINGS -------------"
        private const val GooglePaySettingsHeader: String = "----------- GOOGLE PAY SETTINGS -------------"
    }
}
