...
class OloPayTestActivity : AppCompatActivity() {
    ...
    // NOTE: Using navGraph.nodes is apparently a restricted API, but it wasn't flagged until after
    // updating to Kotlin 2.2 and compiling with API 36. Eventually we need to replace this with
    // different logic, but it's not exactly trivial. Since this is just a test app that isn't
    // going into production, we're just going to leave this for now and fix it if it starts causing
    // issues
