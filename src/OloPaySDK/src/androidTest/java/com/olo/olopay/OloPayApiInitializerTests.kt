...
class OloPayApiInitializerTests {
    ...
    @Test
    fun setup_withFreshSetup_testPublishableKeyWithValue_testPublishableKeyUpdated() {
        Storage(testContext).testPublishableKey = "Foobar"

        runBlocking {
            OloPayApiInitializer().setup(testContext, SetupParameters(OloPayEnvironment.Test, true))
            assertNotEquals("Foobar", Storage(testContext).testPublishableKey)
        }
    }

    @Test
    fun setup_withFreshSetup_testPublishableKeyNull_testPublishableKeyUpdated() {
    ...
    @Test
    fun setup_withFreshSetup_testPublishableKeyEmpty_testPublishableKeyUpdated() {
    ...
    @Test
    fun setup_withoutFreshSetup_withCachedTestPublishableKey_testPublishableKeyNotChanged() {
        Storage(testContext).testPublishableKey = "Foobar"

        runBlocking {
            OloPayApiInitializer().setup(testContext, SetupParameters(OloPayEnvironment.Test, false))
            assertEquals("Foobar", Storage(testContext).testPublishableKey)
        }
    }

    @Test
    fun setup_withoutFreshSetup_testPublishableKeyNull_testPublishableKeyUpdated() {
        Storage(testContext).testPublishableKey = null
    ...
    @Test
    fun setup_withoutFreshSetup_testPublishableKeyEmpty_testPublishableKeyUpdated() {
        Storage(testContext).testPublishableKey = ""
