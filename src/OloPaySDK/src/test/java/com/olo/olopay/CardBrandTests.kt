...
class CardBrandTests {
    ...
    @Test
    fun convertFrom_jcbStripeBrand_returnsOloPayBrand_discover() {
        assertEquals(CardBrand.Discover, CardBrand.convertFrom(StripeCardBrand.JCB))
    }

    @Test
    fun convertFrom_dinersClubStripeBrand_returnsOloPayBrand_discover() {
        assertEquals(CardBrand.Discover, CardBrand.convertFrom(StripeCardBrand.DinersClub))
    }

    @Test
    fun convertFrom_unionPayStripeBrand_returnsOloPayBrand_discover() {
        assertEquals(CardBrand.Discover, CardBrand.convertFrom(StripeCardBrand.UnionPay))
    }
    ...
    @Test
    fun convertFrom_cartesBancairesString_returnsOloPayBrand_unsupported() {
        assertEquals(CardBrand.Unsupported, CardBrand.convertFrom("CARTESBANCAIRES"))
        assertEquals(CardBrand.Unsupported, CardBrand.convertFrom("cartesbancaires"))
        assertEquals(CardBrand.Unsupported, CardBrand.convertFrom("CaRtEsBaNcAiReS"))
    }

    @Test
    fun convertFrom_jcbString_returnsOloPayBrand_discover() {
        assertEquals(CardBrand.Discover, CardBrand.convertFrom("JCB"))
        assertEquals(CardBrand.Discover, CardBrand.convertFrom("jcb"))
        assertEquals(CardBrand.Discover, CardBrand.convertFrom("JcB"))
    }

    @Test
    fun convertFrom_dinersClubString_returnsOloPayBrand_discover() {
        assertEquals(CardBrand.Discover, CardBrand.convertFrom("DINERSCLUB"))
        assertEquals(CardBrand.Discover, CardBrand.convertFrom("dinersclub"))
        assertEquals(CardBrand.Discover, CardBrand.convertFrom("DiNeRsClUb"))
    }

    @Test
    fun convertFrom_unionPayString_returnsOloPayBrand_discover() {
        assertEquals(CardBrand.Discover, CardBrand.convertFrom("UNIONPAY"))
        assertEquals(CardBrand.Discover, CardBrand.convertFrom("unionpay"))
        assertEquals(CardBrand.Discover, CardBrand.convertFrom("uNiOnPaY"))
    }
