...
internal class GooglePayLauncherViewModel constructor(
    ...
    internal suspend fun createPaymentMethod(context: Context, paymentData: PaymentData): GooglePayResult {
        val paymentJson = JSONObject(paymentData.toJson())
        val params = PaymentMethodParams(PaymentMethodCreateParams.createFromGooglePay(paymentJson), PaymentMethodSource.GooglePay)
