...
class GooglePayContext: IGooglePayContext {
    ...
    private val _launcher: GooglePayPaymentMethodLauncher
    ...
    constructor(
        ...
        _launcher = GooglePayPaymentMethodLauncher(
            activity,
            getGooglePayConfig(merchantName, merchantCountryCode),
     ...
     */
    override var readyCallback: ReadyCallback?
    ...
    override fun present(currencyCode: String, amount: Int, transactionId: String?) {
        ...
        _launcher.present(currencyCode, amount, transactionId)
    }
    ...
    private fun getGooglePayConfig(
        merchantName: String? = null,
        merchantCountryCode: String? = null
    ) : GooglePayPaymentMethodLauncher.Config {
        val oloPayConfig = IOloPayApiInitializer.googlePayConfig
        if (oloPayConfig == null) {
            val message = "Google Pay is not configured. Call OloPayAPI.init() with Google Pay configuration parameters"
            throw GooglePayException(IllegalStateException(message), GooglePayErrorType.InternalError)
        }

        return GooglePayPaymentMethodLauncher.Config(
            Environment.convertFrom(oloPayConfig.environment),
            merchantCountryCode ?: oloPayConfig.companyCountryCode,
            merchantName ?: oloPayConfig.companyName,
            oloPayConfig.emailRequired,
            GooglePayPaymentMethodLauncher.BillingAddressConfig(
                true,
                oloPayConfig.addressFormat.toStripeFormat(),
                oloPayConfig.phoneNumberRequired
            ),
            oloPayConfig.existingPaymentMethodRequired
        )
    }
    ...
    private fun onGooglePayResult(result: GooglePayPaymentMethodLauncher.Result) {
        when(result) {
            is GooglePayPaymentMethodLauncher.Result.Completed -> {
                resultCallback?.onResult(Result.Completed(PaymentMethod(result.paymentMethod)))
            }
            GooglePayPaymentMethodLauncher.Result.Canceled -> {
                resultCallback?.onResult(Result.Canceled)
            }
            is GooglePayPaymentMethodLauncher.Result.Failed -> {
                val errorType = GooglePayErrorType.from(result.errorCode)
                resultCallback?.onResult(Result.Failed(GooglePayException(result.error, errorType)))
            }
        }
    }
}