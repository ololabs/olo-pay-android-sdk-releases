...
import com.olo.olopay.api.IOloPayApiInitializer.GooglePay.environment
import com.olo.olopay.api.IOloPayApiInitializer.GooglePay.googlePayConfig
...
class OloPayApiInitializer : IOloPayApiInitializer {
     ...
     * #### Important:
     *
     * _This method is provided mainly as convenience for Java developers. Kotlin developers should
     * generally use the suspend version of this method instead_
     *
    ...
    override suspend fun setup(context: Context, parameters: SetupParameters?) {
        try {
            val publishableKey = Storage(context).publishableKey

            googlePayConfig = parameters?.googlePayConfig
            environment = parameters?.environment ?: OloPayEnvironment.Production

            if (parameters?.freshSetup == true || publishableKey.isNullOrEmpty()) {
